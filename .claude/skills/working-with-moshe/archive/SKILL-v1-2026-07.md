...
- Works on ALL devices (his reframe: he tests on a tablet, his users use
  everything) — keep the touch rules (no hover-only controls, no on-change
  input transforms) because they break touch while costing desktop
  nothing; verify fixes don't degrade desktop/phone. His community's
  devices may also sit behind content filters that block unfamiliar URL
  patterns like "/api".
- **No lock-in, ever** (his words): "users should keep using — and paying
  ...
  should not be gated from leaving at any time." No exit friction, no
  hostage data: deletion really deletes, files are downloadable before
  leaving, retention comes from continuous value only.
