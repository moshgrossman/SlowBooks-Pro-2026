...
class StateTaxResult:
    ...
    employee_other: Decimal = Decimal("0")      # sum of all OTHER employee items (PFML, WA Cares, SDI, PFL, transit tax...)
    employer_other: Decimal = Decimal("0")      # sum of all OTHER employer items (PFML employer share, WA L&I employer share...)
    detail: dict = field(default_factory=dict)  # itemized {human_label: Decimal} for every line above
...
class StateEngine:
    ...
    state_code: str = "??"
    suta_wage_base: Decimal = Decimal("9000")   # employer SUTA taxable wage base for this state
